...
def dfs_visit(r: int, edges: list):  # r == 퀸을 두기 위해 탐색하는 행
    ...
    if r == N:
        ...
        return
        # 지금까지 두었던 자리들과 비교
    # for search_row, positioned in enumerate(edges[:r]):
    for col in range(N):    # 컬럼 탐색
        if (col in edges[:r]):  # 두려고 하는 행 보다 위에, 같은 줄에 위치한 경우
            continue
        # if (abs(r-search_row) == abs(col-positioned)):  # 대각선에 위치한 경우
        for i, v in enumerate(edges[:r]):
            if (i == v) and (r == col):  # 대각선에 위치한 경우
                break  # 여기서 goto else 처럼 작동할 수 있었으면 좋겠는데.?!!!
        else:
            edges[r] = col
            # print(edges)
            dfs_visit(r+1, edges)
