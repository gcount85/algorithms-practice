...
def bfs_for_고슴도치(s: tuple, 지도: list, water_level: dict):
        ...
        for u in frontier:          # frontier에 있는 시작노드 s에 대해
            ...
            for v in edges:      
                if (0 <= v[0] <= R-1) and (0 <= v[1] <= C-1):  # 목적지 노드의 조건
                    stuff = 지도[v[0]][v[1]]
                    if (v not in level) and ((stuff == '.') or (stuff == 'D')) and (water_level[v] > i):  
...
def bfs_for_water(water: tuple, 지도: list):
    water_level = {water: 0}      # 단계(level)은 몇 스텝만에 시작노드에서 해당 노드로 갈 수 있는지를 의미
    i = 1               # 단계를 나타낼 i (다음 단계를 탐색할 때마다 +1)
...
bfs_for_고슴도치(start, 지도, water_level)